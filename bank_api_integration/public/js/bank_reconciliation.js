...
frappe.pages['bank-reconciliation'].on_page_refresh = function(wrapper) {
...
erpnext.accounts.bankReconciliation = class BankReconciliation {
	...
	make() {
		...
		const empty_state = __("Sync,link or reconcile a bank account")
	...
	check_banking_api_status() {
		const me = this;
		frappe.db.get_value("Bank API Integration Settings", "Bank API Integration Settings", "enabled", (r) => {
			if (r && r.enabled === "1") {
				me.integration_status = "active"
			} else {
				me.integration_status = "inactive"
			}
		})
	}
	...
	add_actions() {
		...
		me.page.show_menu()
		if (me.integration_status==="active") {
			me.page.add_menu_item(__("Synchronize bank account"), function() {
...
erpnext.accounts.bankTransactionSync = class bankTransactionSync {
	...
	sync_transactions() {
		const me = this;
			frappe.xcall('bank_api_integration.bank_api_integration.doctype.bank_statement.bank_statement.sync_transactions', {
				freeze: true
            })
			.then((result) => {
